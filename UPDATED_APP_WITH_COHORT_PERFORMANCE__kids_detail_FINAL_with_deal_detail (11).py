...
def _render_marketing_deal_detail(df):
    ...
    # Trim columns locally for safety
    try:
        df = df.copy()
        df.columns = [str(c).strip() for c in df.columns]
    except Exception:
    ...
    pay_col = _col(df, ["Payment Received Date","Payment Received date","Enrollment Date","Enrolment Date","Enrolled On","Payment Date","Payment_Received_Date","Payment Received Date "])
    ...
    # ---- Day-first parsing with fallbacks
    def _to_dt(s: pd.Series):
    ...
    # ---- Deal Name options limited to created-in-window deals
    ...
    sel_deals = st.multiselect("Deal Name", deal_opts, default=deal_opts[:1], key="dd_deal")
    if not sel_deals:
    ...
    # ---- Denominator for % (all creates in window within the selected sources)
    if source_col:
        if src_set:
    ...
    # Helpful hint if zero enrolments
    if enrol_count == 0:
        last_pay = _P_any[sel_mask].dropna().max()
        if pd.notna(last_pay):
            st.caption(f"Latest payment among selected deals is on {str(last_pay.date())} — may be outside the selected window.")
    ...
    # ---- Per-deal breakdown (if multi-select, show one row per selected deal)
    try:
        rows = []
        for dname in sel_deals:
            ...
            # Denominator per deal: creates in window for that deal's source
    ...
    # Column resolver for contact fields
    ...
    if not (parent_phone_col or parent_email_col):
        ...
        phone_yes = 0
        if parent_phone_col:
            _phone = df[parent_phone_col].astype(str).str.strip()
            _phone = _phone.fillna("")
            # Consider non-empty and contains at least one digit as "available"
        ...
        email_yes = 0
        if parent_email_col:
            _email = df[parent_email_col].astype(str).str.strip()
            _email = _email.fillna("")
