...
def _render_marketing_half_baked(df):
    ...
    deal_col = find_col(df, ["Deal Name","Deal name","Name","Deal","Title"])
    source_col = find_col(df, ["JetLearn Deal Source","Original Source","Original Source 1","Deal Source","Source"])
    create_col = find_col(df, ["Create Date","Created Date","Created On","Creation Date","Deal Created Date","Create_Date"])
    pay_col = find_col(df, ["Payment Received Date","Enrollment Date","Payment Date","Payment_Received_Date","Enrolled On"])
    calib_book = find_col(df, ["[Trigger] - Calibration Booking Date","Calibration Booking Date","Calibration Booked Date","Calibration Scheduled Date","Calibration Slot (Deal)"])
    calib_res  = find_col(df, ["[Trigger] - Calibration Rescheduled Date","Calibration Rescheduled Date","Calib Rescheduled Date"])
    calib_done = find_col(df, ["[Trigger] - Calibration Done Date","Calibration Done Date","Calibration Completed Date"])

    email_col = find_col(df, ["Parent Email","Parent Email ID","Email","Parent Email Address"])
    phone_col = find_col(df, ["Parent Phone number","Parent Phone","Phone","Parent Mobile"])

    if deal_col is None or source_col is None:
        st.warning("Missing required columns (Deal Name or JetLearn Deal Source)."); return

    # Deal selector
    all_deals = df[deal_col].dropna().astype(str).unique().tolist()
    all_deals = sorted(all_deals)
    sel = st.multiselect("Deal Name (pick one or many)", options=all_deals, default=all_deals, key="hb_deal_sel")

    df_sel = df[df[deal_col].astype(str).isin(sel)] if sel else df.iloc[0:0]
    if df_sel.empty:
        st.info("No rows for the selected deal names."); return

    # Build helpers
    def _non_null_date(col):
        if col is None: 
            return pd.Series(False, index=df_sel.index)
        s = df_sel[col]
    ...
    enrolled = _non_null_date(pay_col)
    booked   = _non_null_date(calib_book)
    resched  = _non_null_date(calib_res)
    done     = _non_null_date(calib_done)

    # Overall vs selected source mix (counts of deals)
    src_sel_counts = df_sel[source_col].fillna("Unknown").value_counts(dropna=False)
    src_all_counts = df[source_col].fillna("Unknown").value_counts(dropna=False)

    total_sel = int(src_sel_counts.sum())
    total_all = int(src_all_counts.sum())

    rows = []
    for src in sorted(set(src_sel_counts.index).union(set(src_all_counts.index))):
        sel_cnt = int(src_sel_counts.get(src, 0))
        all_cnt = int(src_all_counts.get(src, 0))
        pct_sel = (sel_cnt / total_sel * 100.0) if total_sel else 0.0
        pct_all = (all_cnt / total_all * 100.0) if total_all else 0.0

        # Metrics within the selected subset & this source
        mask_src = df_sel[source_col].fillna("Unknown").astype(str) == str(src)
        src_rows = df_sel[mask_src]
        # recompute booleans aligned to src_rows
        def _mask_non_null(rows, col):
            if col is None: return 0
            try:
                return coerce_datetime(rows[col]).notna().sum()
            except Exception:
                return (rows[col].astype(str).str.strip() != "").sum()

        enroll_cnt = _mask_non_null(src_rows, pay_col)
        book_cnt   = _mask_non_null(src_rows, calib_book)
        res_cnt    = _mask_non_null(src_rows, calib_res)
        done_cnt   = _mask_non_null(src_rows, calib_done)

        # contactability
        def _avail(rows, col):
            if col is None: return (0, 0.0)
            vals = rows[col].astype(str).str.strip()
            c = (vals != "").sum()
            p = (c / len(rows) * 100.0) if len(rows) else 0.0
            return (int(c), p)

        email_c, email_p = _avail(src_rows, email_col)
        phone_c, phone_p = _avail(src_rows, phone_col)

        rows.append({
            "JetLearn Deal Source": src,
            "Deals (selected)": sel_cnt,
            "% in selected": pct_sel,
            "% overall": pct_all,
            "Δ (pp)": pct_sel - pct_all,
            "Enrollments": int(enroll_cnt),
            "First Calibration Scheduled": int(book_cnt),
            "Calibration Rescheduled": int(res_cnt),
            "Calibration Done": int(done_cnt),
            "Parent Email Available (count)": int(email_c),
            "Parent Email Available (%)": email_p,
            "Parent Phone Available (count)": int(phone_c),
            "Parent Phone Available (%)": phone_p,
        })

    out = pd.DataFrame(rows).sort_values(by="Deals (selected)", ascending=False).reset_index(drop=True)

    st.subheader("Half Baked — Source Mix vs Overall & Funnel Health")
    st.caption("Compare the source composition of your selected deals against overall, and inspect calibration/enrollment progress and contactability.")

    st.dataframe(out, use_container_width=True)

    # Chart: % in selected vs % overall
    try:
        import altair as alt
        chart_data = out.melt(id_vars=["JetLearn Deal Source"], value_vars=["% in selected","% overall"], var_name="Metric", value_name="Percent")
        chart = alt.Chart(chart_data).mark_bar().encode(
            x=alt.X("JetLearn Deal Source:N", sort="-y"),
            y=alt.Y("Percent:Q"),
            column="Metric:N",
            tooltip=["JetLearn Deal Source","Percent"]
        ).properties(
            width=280,
            height=300
        )
    ...
    st.download_button("Download CSV — Half Baked Funnel", data=csv, file_name="half_baked_funnel.csv", mime="text/csv")
