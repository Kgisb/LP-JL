...
# === Half Baked renderer (APPENDED SAFELY) ===
def _render_marketing_half_baked(df):
    ...
    if (deal_name_col is None) or (source_col is None):
    ...
    selected = st.multiselect("Deal Name (leave blank for All)", names, key="hb_deal_names")

    df_all = df.copy()
    if selected:
        df_sel = df_all[df_all[deal_name_col].astype(str).isin(selected)].copy()
    else:
        df_sel = df_all.copy()
    ...
    for c in [enroll_col, cal_book_col, cal_slot_col, cal_resch_col, cal_done_col]:
        if c and c in df_sel.columns:
            df_sel[c] = _to_dt(df_sel[c])
    if enroll_col and enroll_col in df_all.columns:
        df_all[enroll_col] = _to_dt(df_all[enroll_col])

    def first_cal_scheduled(row):
        v1 = row.get(cal_book_col) if cal_book_col else None
        v2 = row.get(cal_slot_col) if cal_slot_col else None
        return v1 if _pd.notna(v1) else v2

    has_enrollment = df_sel[enroll_col].notna() if enroll_col else _pd.Series(False, index=df_sel.index)
    has_cal_sched  = df_sel.apply(first_cal_scheduled, axis=1).notna() if (cal_book_col or cal_slot_col) else _pd.Series(False, index=df_sel.index)
    has_resched    = df_sel[cal_resch_col].notna() if cal_resch_col else _pd.Series(False, index=df_sel.index)
    has_done       = df_sel[cal_done_col].notna() if cal_done_col else _pd.Series(False, index=df_sel.index)

    def _nonempty(s):
        return s.astype(str).str.strip().ne("").fillna(False)
    has_email = _nonempty(df_sel[email_col]) if email_col else _pd.Series(False, index=df_sel.index)
    has_phone = _nonempty(df_sel[phone_col]) if phone_col else _pd.Series(False, index=df_sel.index)

    grp_sel = df_sel.groupby(source_col, dropna=False)
    sel_counts = grp_sel.size().rename("Deals (selected)").astype(int)

    grp_all = df_all.groupby(source_col, dropna=False)
    all_counts = grp_all.size().rename("Deals (overall)").astype(int)

    total_sel = int(sel_counts.sum())
    total_all = int(all_counts.sum())

    enroll_counts      = grp_sel[has_enrollment].sum().rename("Enrollments").astype(int) if not has_enrollment.empty else _pd.Series(dtype=int)
    cal_sched_counts   = grp_sel[has_cal_sched].sum().rename("First Calibration Scheduled").astype(int) if not has_cal_sched.empty else _pd.Series(dtype=int)
    cal_resch_counts   = grp_sel[has_resched].sum().rename("Calibration Rescheduled").astype(int) if not has_resched.empty else _pd.Series(dtype=int)
    cal_done_counts    = grp_sel[has_done].sum().rename("Calibration Done").astype(int) if not has_done.empty else _pd.Series(dtype=int)
    email_counts       = grp_sel[has_email].sum().rename("Parent Email Available (count)").astype(int) if not has_email.empty else _pd.Series(dtype=int)
    phone_counts       = grp_sel[has_phone].sum().rename("Parent Phone Available (count)").astype(int) if not has_phone.empty else _pd.Series(dtype=int)
    ...
    cols = ["JetLearn Deal Source"] + [c for c in preferred if c in out.columns] + [c for c in out.columns if c not in preferred and c != "JetLearn Deal Source"]
    out = out[cols]

    num_cols = out.select_dtypes(include="number").columns
    totals = {c: out[c].sum() for c in num_cols}
...
# --- Half Baked dispatch (appended) ---
try:
    if 'section' in globals() and 'view' in globals():
        if str(section).strip() == "Marketing" and str(view).strip() == "Half Baked":
            _render_marketing_half_baked(df)
except Exception as _hb_ex:
    import streamlit as st
    st.error(f"Half Baked error: {type(_hb_ex).__name__}: {_hb_ex}")
