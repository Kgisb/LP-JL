...
# === Half Baked renderer ===
def _render_marketing_half_baked(df):
    import pandas as _pd, numpy as _np, re as _re, streamlit as st, altair as alt

    if df is None or getattr(df, "empty", True):
        st.info("No data available."); return

    # ---------- helpers ----------
    def _col_exists(_df, candidates):
        # exact
        for c in candidates:
            if c in _df.columns: return c
        # tolerant: lower/strip matching
        low = {c.lower().strip(): c for c in _df.columns}
        for c in candidates:
            key = c.lower().strip()
            if key in low: return low[key]
        # fallback: substring contains (case-insensitive)
    ...
    # ---------- resolve columns ----------
    deal_name_col = _col_exists(df, ["Deal Name", "Deal name", "D name", "Name of deal", "Deal"])
    source_col    = _col_exists(df, ["JetLearn Deal Source","Jetlearn Deal Source","Deal Source","Source"])
    enroll_col    = _col_exists(df, ["Payment Received Date","Payment Received Date "])
    cal_book_col  = _col_exists(df, ["[Trigger] - Calibration Booking Date","Calibration Booking Date"])
    cal_slot_col  = _col_exists(df, ["Calibration Slot (Deal)"])
    cal_resch_col = _col_exists(df, ["[Trigger] - Calibration Rescheduled Date","Calibration Rescheduled Date"])
    cal_done_col  = _col_exists(df, ["[Trigger] - Calibration Done Date","Calibration Done Date"])
    email_col     = _col_exists(df, ["Parent Email","Parent Email "])
    phone_col     = _col_exists(df, ["Parent Phone number","Parent Phone Number","Parent Phone no.","Parent Phone"])

    # fail fast if critical columns missing
    missing = [x for x in [deal_name_col, source_col] if x is None]
    if missing:
        st.warning("Required columns missing for Half Baked (need Deal Name and JetLearn Deal Source)."); return

    # ---------- selections ----------
    st.subheader("Half Baked — Funnel & Hygiene by Source")
    names = _pd.Series(df[deal_name_col].dropna().astype(str).unique()).sort_values().tolist()
    selected = st.multiselect("Deal Name (blank = All)", names, key="hb_deal_names")

    # make working copies
    dfw_all = df.copy()
    # Selected subset
    if selected:
        dfw = dfw_all[dfw_all[deal_name_col].astype(str).isin(selected)].copy()
    else:
        dfw = dfw_all.copy()

    if dfw.empty:
        st.info("No rows after selection."); return

    # ---------- date coercions for metrics ----------
    for c in [enroll_col, cal_book_col, cal_slot_col, cal_resch_col, cal_done_col]:
        if c and c in dfw.columns:
            dfw[c] = _to_dt(dfw[c])
    if enroll_col and enroll_col in dfw_all.columns:
        dfw_all[enroll_col] = _to_dt(dfw_all[enroll_col])

    # pick calibration first booking date (trigger preferred, else slot)
    def first_cal_scheduled(row):
        v1 = _pd.NaT if cal_book_col is None else row.get(cal_book_col, _pd.NaT)
        v2 = _pd.NaT if cal_slot_col is None else row.get(cal_slot_col, _pd.NaT)
        if _pd.notna(v1): return v1
        return v2

    has_enrollment = dfw[enroll_col].notna() if enroll_col else _pd.Series(False, index=dfw.index)
    has_cal_sched  = dfw.apply(first_cal_scheduled, axis=1).notna() if (cal_book_col or cal_slot_col) else _pd.Series(False, index=dfw.index)
    has_resched    = dfw[cal_resch_col].notna() if cal_resch_col else _pd.Series(False, index=dfw.index)
    has_done       = dfw[cal_done_col].notna() if cal_done_col else _pd.Series(False, index=dfw.index)

    has_email = dfw[email_col].astype(str).str.strip().ne("").fillna(False) if email_col else _pd.Series(False, index=dfw.index)
    has_phone = dfw[phone_col].astype(str).str.strip().ne("").fillna(False) if phone_col else _pd.Series(False, index=dfw.index)

    # ---------- group & compute ----------
    # Selected scope counts
    grp = dfw.groupby(source_col, dropna=False)
    sel_counts = grp.size().rename("Deals (selected)").astype(int)

    # Overall % baseline (same filters except Deal Name)
    grp_all = dfw_all.groupby(source_col, dropna=False)
    all_counts = grp_all.size().rename("Deals (overall)").astype(int)
    total_sel = sel_counts.sum()
    total_all = all_counts.sum()

    # per-source metrics in selected
    enroll_counts = grp[has_enrollment].sum().rename("Enrollments").astype(int) if not has_enrollment.empty else _pd.Series(dtype=int)
    cal_sched_counts = grp[has_cal_sched].sum().rename("First Calibration Scheduled").astype(int) if not has_cal_sched.empty else _pd.Series(dtype=int)
    cal_resch_counts = grp[has_resched].sum().rename("Calibration Rescheduled").astype(int) if not has_resched.empty else _pd.Series(dtype=int)
    cal_done_counts  = grp[has_done].sum().rename("Calibration Done").astype(int) if not has_done.empty else _pd.Series(dtype=int)

    email_counts = grp[has_email].sum().rename("Parent Email Available (count)").astype(int) if not has_email.empty else _pd.Series(dtype=int)
    phone_counts = grp[has_phone].sum().rename("Parent Phone Available (count)").astype(int) if not has_phone.empty else _pd.Series(dtype=int)

    # assemble table
    out = _pd.concat([
        sel_counts,
        enroll_counts, cal_sched_counts, cal_resch_counts, cal_done_counts,
        email_counts, phone_counts,
        all_counts
    ], axis=1).fillna(0)

    # percentages
    out["% in selected"] = _np.where(total_sel>0, out["Deals (selected)"]*100.0/total_sel, 0.0)
    out["% overall"]     = _np.where(total_all>0, out["Deals (overall)"]*100.0/total_all, 0.0)
    out["Δ (pp)"]        = out["% in selected"] - out["% overall"]

    # email/phone % relative to selected rows per source
    out["Parent Email Available (%)"] = _np.where(out["Deals (selected)"]>0, out["Parent Email Available (count)"]*100.0/out["Deals (selected)"], 0.0)
    out["Parent Phone Available (%)"] = _np.where(out["Deals (selected)"]>0, out["Parent Phone Available (count)"]*100.0/out["Deals (selected)"], 0.0)

    # order columns
    col_order = [
        "Deals (selected)",
        "% in selected", "% overall", "Δ (pp)",
        "Enrollments", "First Calibration Scheduled", "Calibration Rescheduled", "Calibration Done",
        "Parent Email Available (count)", "Parent Email Available (%)",
        "Parent Phone Available (count)", "Parent Phone Available (%)",
        "Deals (overall)",
    ]
    # ensure present
    ...
    # pretty index
    out = out.reset_index().rename(columns={source_col: "JetLearn Deal Source"})
    out = out[col_order if set(col_order).issubset(out.columns) else out.columns.tolist()]

    # totals row
    def _safe_sum(col): 
        return out[col].sum() if _pd.api.types.is_numeric_dtype(out[col]) else _pd.NA
    totals = {c: _safe_sum(c) for c in out.columns if c != "JetLearn Deal Source"}
    totals_row = _pd.DataFrame([{"JetLearn Deal Source": "Total", **totals}])
    out_totalled = _pd.concat([out, totals_row], ignore_index=True)

    st.caption("Funnel by source for selected Deal Name(s) vs overall baseline")
    st.dataframe(out_totalled, use_container_width=True)

    # ---------- chart ----------
    chart_df = out[out["JetLearn Deal Source"]!="Total"][["JetLearn Deal Source","% in selected","% overall"]]
    chart_df = chart_df.melt(id_vars="JetLearn Deal Source", var_name="Series", value_name="Percent")
    c = alt.Chart(chart_df).mark_bar().encode(
        x=alt.X('JetLearn Deal Source:N', sort='-y', title='JetLearn Deal Source'),
    ...
    # ---------- download ----------
    st.download_button(
        "Download Half Baked Funnel (CSV)",
        out_totalled.to_csv(index=False).encode("utf-8"),
        file_name="half_baked_funnel.csv",
        mime="text/csv",
        key="hb_dl_funnel_csv"
    )
...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Half Baked"],
    }
...
# --- Half Baked dispatch ---
if view == "Half Baked":
    _render_marketing_half_baked(df)
# --- /Half Baked dispatch ---
