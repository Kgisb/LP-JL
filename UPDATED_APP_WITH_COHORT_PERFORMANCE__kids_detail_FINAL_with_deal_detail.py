...
def _render_marketing_deal_detail(df):
    ...
    pay_col = _col(df, ["Payment Received Date","Enrollment Date","Enrolment Date","Enrolled On","Payment Date","Payment_Received_Date"])
    source_col = _col(df, ["JetLearn Deal Source","Deal Source","Original source","Source","Original traffic source"])

    if not deal_col or not create_col or not pay_col:
        st.error("Could not find required columns (Deal Name / Create Date / Payment Received Date).")
        return
    ...
    # Counts
    # Deal Count: unique Deal Name rows for selected deal created within window
    deal_count = int((m_create & sel_mask).sum())  # usually 1 if unique names
    
    # Enrollment Count: payments in window (cohort/MTD logic) for the selected deal
    enrol_count = int((enrol_mask & sel_mask).sum())

    # Percentage of selected Deal against its JetLearn Deal Source
    # denom: total deals created in window that belong to src_for_sel
...
try:
    ...
    import streamlit as st as _st  # avoid shadowing
