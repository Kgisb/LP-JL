...
# === Deal type (with Funnel) renderer — PREPENDED ===
def _dt_safe_to_datetime(_s):
    try:
        return _pd.to_datetime(_s, errors="coerce", infer_datetime_format=True, dayfirst=True)
    except Exception:
        return _pd.to_datetime(_s, errors="coerce")

def _deal_type_find_col(df: _pd.DataFrame, cands: List[str]):
    # Prefer global find_col if it exists in the app
    try:
        if 'find_col' in globals() and callable(globals()['find_col']):
            return globals()['find_col'](df, cands)
    except Exception:
        pass
    # Fallback resolver (case-insensitive exact + contains)
    for c in cands:
        if c in df.columns:
            return c
...
def _render_performance_deal_type(
    ...
    trial_scheduled_col: str | None = None,        # maps to First Calibration Scheduled
    trial_rescheduled_col: str | None = None,      # (kept but not shown in table per latest ask)
    calibration_booked_col: str | None = None,     # Booking (kept but not shown)
    calibration_done_col: str | None = None,       # Done
    calibration_scheduled_col: str | None = None,  # explicit "Calibration Scheduled Date" (non-first) if exists
):
    import streamlit as st

    st.subheader("Performance — Deal type (Funnel + Enrolments)")
    ...
    # Milestones (we keep broader lists for resilience)
    first_cal_sched_col = trial_scheduled_col or _deal_type_find_col(df_f, [
        "First Calibration Scheduled Date","Trial Scheduled Date","First Trial Scheduled Date","First_Calibration_Scheduled_Date","first calibration scheduled date"
    ])
    cal_sched_col = calibration_scheduled_col or _deal_type_find_col(df_f, [
        "Calibration Scheduled Date","calibration scheduled date","Calibration_Scheduled_Date"
    ])
    cal_done_col_res = calibration_done_col or _deal_type_find_col(df_f, [
        "Calibration Done Date","Trial Done Date","Trial Completed Date","Calibration Completed Date","Calibration_Done_Date"
    ])
    ...
    _first_cal_dt = _dt_safe_to_datetime(df_f[first_cal_sched_col]) if (first_cal_sched_col and first_cal_sched_col in df_f.columns) else _pd.Series(_pd.NaT, index=df_f.index)
    _cal_sched_dt  = _dt_safe_to_datetime(df_f[cal_sched_col])      if (cal_sched_col and cal_sched_col in df_f.columns)             else _pd.Series(_pd.NaT, index=df_f.index)
    _cal_done_dt   = _dt_safe_to_datetime(df_f[cal_done_col_res])   if (cal_done_col_res and cal_done_col_res in df_f.columns)       else _pd.Series(_pd.NaT, index=df_f.index)
    ...
    in_range_enroll   = _pay_dt.between(start_dt, end_dt) if _pay_dt.notna().any() else _pd.Series(False, index=df_f.index)
    in_range_firstcal = _first_cal_dt.between(start_dt, end_dt) if _first_cal_dt.notna().any() else _pd.Series(False, index=df_f.index)
    in_range_calsched = _cal_sched_dt.between(start_dt, end_dt) if _cal_sched_dt.notna().any() else _pd.Series(False, index=df_f.index)
    in_range_caldone  = _cal_done_dt.between(start_dt, end_dt) if _cal_done_dt.notna().any() else _pd.Series(False, index=df_f.index)

    # Base selection by Deal Name
    base = df_f[deal_col].astype(str).isin(picked)

    def _mode_mask(mask_series: _pd.Series):
        if mode == "MTD":
            return base & in_create_month & mask_series
        else:
            return base & mask_series

    # Any rows to compute per-source deal counts (respecting MTD create-month in MTD mode)
    ...
    d["_src"]  = d[src_col].fillna("Unknown").astype(str).str.strip()

    # Unique deals baseline by source
    dedup_src_deal = d.drop_duplicates(["_src","_deal"]).copy()
    deals_per_src = dedup_src_deal.groupby("_src")["_deal"].nunique().rename("Deals")

    # Parent phone/email yes/no counts per source (unique deals)
    has_phone_deals = d.drop_duplicates("_deal")
    has_phone_deals["_has_phone"] = has_phone_deals[phone_col].astype(str).str.strip().ne("") if (phone_col and phone_col in has_phone_deals.columns) else False
    has_email_deals = d.drop_duplicates("_deal")
    has_email_deals["_has_email"] = has_email_deals[email_col].astype(str).str.strip().ne("") if (email_col and email_col in has_email_deals.columns) else False

    yes_phone_set = set(has_phone_deals.loc[has_phone_deals["_has_phone"], "_deal"])
    yes_email_set = set(has_email_deals.loc[has_email_deals["_has_email"], "_deal"])

    def count_yes_no_per_src(src_group: _pd.DataFrame, yes_set: set):
        u = src_group.drop_duplicates("_deal")["_deal"]
        yes = int(u.isin(yes_set).sum())
        no  = int(u.shape[0] - yes)
        return yes, no

    phone_yes_no = dedup_src_deal.groupby("_src").apply(lambda g: count_yes_no_per_src(g, yes_phone_set))
    email_yes_no = dedup_src_deal.groupby("_src").apply(lambda g: count_yes_no_per_src(g, yes_email_set))

    phone_yes = phone_yes_no.apply(lambda t: t[0]).rename("Phone Yes")
    phone_no  = phone_yes_no.apply(lambda t: t[1]).rename("Phone No")
    email_yes = email_yes_no.apply(lambda t: t[0]).rename("Email Yes")
    email_no  = email_yes_no.apply(lambda t: t[1]).rename("Email No")

    # Milestone unique-deal counts per source (Enrollment & calibrations)
    def _milestone_counts(milestone_mask: _pd.Series, label: str):
        dd = df_f.loc[_mode_mask(milestone_mask) & df_f[deal_col].astype(str).isin(dedup_src_deal["_deal"]), [deal_col, src_col]].copy()
        if dd.empty:
            return _pd.Series(dtype=int, name=label)
        dd["_deal"] = dd[deal_col].astype(str)
        dd["_src"]  = dd[src_col].fillna("Unknown").astype(str).str.strip()
        dd = dd.drop_duplicates(["_src","_deal"])
        return dd.groupby("_src")["_deal"].nunique().rename(label)

    enroll_counts    = _milestone_counts(in_range_enroll,   "Enrollment")
    first_cal_counts = _milestone_counts(in_range_firstcal, "First Calibration Scheduled")
    cal_sched_counts = _milestone_counts(in_range_calsched, "Calibration Scheduled")
    cal_done_counts  = _milestone_counts(in_range_caldone,  "Calibration Done")

    # Build output
    out = _pd.concat([
        deals_per_src, enroll_counts, first_cal_counts, cal_sched_counts, cal_done_counts,
        phone_yes, phone_no, email_yes, email_no
    ], axis=1).fillna(0).astype({
        "Deals": int, "Enrollment": int, "First Calibration Scheduled": int,
        "Calibration Scheduled": int, "Calibration Done": int,
        "Phone Yes": int, "Phone No": int, "Email Yes": int, "Email No": int
    }).reset_index().rename(columns={"_src":"JetLearn Deal Source"})

    # % of overall (within selected deals across all sources)
    total_selected_deals = int(deals_per_src.sum()) if len(deals_per_src) else 0
    out["% of Overall (Selected)"] = ((out["Deals"] / total_selected_deals) * 100.0).round(1) if total_selected_deals else 0.0

    # Order
    out = out.sort_values(["Deals","Enrollment","Calibration Done","Calibration Scheduled","First Calibration Scheduled"],
                           ascending=[False,False,False,False,False], kind="stable").reset_index(drop=True)

    # KPIs
    c1,c2,c3,c4,c5 = st.columns(5)
    with c1: st.metric("Selected deal names", int(d.drop_duplicates("_deal").shape[0]))
    with c2: st.metric("Sources hit", int(out.shape[0]))
    with c3: st.metric("Enrollments", int(out["Enrollment"].sum()))
    with c4: st.metric("Cal. Done", int(out["Calibration Done"].sum()))
    with c5: st.metric("Cal. Scheduled", int(out["Calibration Scheduled"].sum()))

    # Table + download
    st.dataframe(out, use_container_width=True, hide_index=True)
    st.download_button(
        "Download CSV — Deal type (enrolments + funnel by source)",
        out.to_csv(index=False).encode("utf-8"),
        file_name="deal_type_enrolments_funnel_by_source.csv",
        mime="text/csv",
        key="dl_dealtype_enrol_funnel_by_source"
    )
def _reset_all_filters_and_cache(preserve_nav: bool = True, clear_fn=None):
    """Reset Streamlit UI state and (optionally) clear caches, while keeping nav keys if requested."""
    import streamlit as st
    try:
    ...
    for k in list(st.session_state.keys()):
        if k in keep_keys:
            continue
        key_l = str(k).lower()
        if any(tag in key_l for tag in rm_tags):
            to_delete.append(k)
    for k in to_delete:
        try:
...
def _render_status_bar(excluded_count: int, excluded_col: str | None, df_len: int, track: str | None):
    import streamlit as st
    try:
        msg = []
        if excluded_count and excluded_count > 0:
            label = excluded_col or 'column'
            msg.append(f"Excluded **{excluded_count:,}** rows where **{label}** was invalid (e.g., '1.2 Invalid deal').")
        if df_len is not None:
            msg.append(f"In-scope rows: **{df_len:,}**")
        if track:
            msg.append(f"Track: **{track}**")
        if msg:
            st.markdown(' &nbsp; | &nbsp; '.join(msg), unsafe_allow_html=True)
...
try:
    if 'master' in globals() and master == "Performance" and 'view' in globals() and view == "Deal type":
        _render_performance_deal_type(
            ...
            calibration_scheduled_col=find_col(df, ["Calibration Scheduled Date","calibration scheduled date","Calibration_Scheduled_Date"]) if 'df' in globals() else None,
        )
except Exception as e:
    import streamlit as st
