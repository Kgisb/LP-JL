...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail"],
    }
...
def _render_marketing_deal_detail(df):
    ...
    # Trim columns locally as extra safety
    try:
        df = df.copy()
        df.columns = [str(c).strip() for c in df.columns]
    except Exception:
    ...
    pay_col = _col(df, ["Payment Received Date","Payment Received date","Enrollment Date","Enrolment Date","Enrolled On","Payment Date","Payment_Received_Date"])
    pay_col_fallback = _col(df, ["Renewal: Payment Received Date","Renewal Payment Received Date"])
    ...
    if not deal_col or not create_col:
        st.error("Could not find required columns (Deal Name / Create Date).")
        return
    if not (pay_col or pay_col_fallback):
        st.warning("Payment date column not found; enrolment will show 0 unless present under renewal column.")

    # ---- Date parsing (day-first tolerant)
    def _to_dt(s: pd.Series):
    ...
    if pay_col:
        _P = _to_dt(df[pay_col])
    else:
        _P = pd.Series(pd.NaT, index=df.index)
    _P_fb = _to_dt(df[pay_col_fallback]) if pay_col_fallback else pd.Series(pd.NaT, index=df.index)

    # Combined payment (primary or fallback)
    _P_any = _P.copy()
    nulls = _P_any.isna()
    if nulls.any():
        _P_any.loc[nulls] = _P_fb.loc[nulls]

    if source_col:
        _SRC = df[source_col].fillna("Unknown").astype(str)
    else:
        _SRC = pd.Series("Unknown", index=df.index)
    ...
    # ---- Deal Name filter (options restricted to rows with Create Date in window)
    in_create = _C.dt.date.between(start_d, end_d)
    deal_opts = sorted(_DEAL.loc[in_create].unique().tolist())
    if not deal_opts:
        st.info("No deals created in the selected window."); return

    sel_deal = st.selectbox("Deal Name", deal_opts, index=0, key="dd_deal")

    # ---- Window masks
    m_create = _C.dt.date.between(start_d, end_d)

    # Enrolment window is by Payment date; for MTD, also require Create in window.
    ...
    # ---- Compute stats restricted to selected deal
    sel_mask = (_DEAL == str(sel_deal))

    # Source for selected deal (within window by Create Date). If multiple, show the most frequent.
    src_for_sel = "Unknown"
    if source_col:
        src_series = _SRC[m_create & sel_mask]
        if len(src_series):
            src_for_sel = src_series.mode().iloc[0] if not src_series.mode().empty else (src_series.iloc[0] if len(src_series) else "Unknown")

    # Counts
    deal_count = int((m_create & sel_mask).sum())
    enrol_count = int((enrol_mask & sel_mask).sum())

    # If enrol_count is zero, offer a hint — show last payment date for this deal (from any payment column)
    hint_msg = None
    if enrol_count == 0:
        last_pay = _P_any[sel_mask].dropna().max()
        if pd.notna(last_pay):
            hint_msg = f"Latest payment for this deal is on {str(last_pay.date())} (may be outside the selected window)."

    # Percentage of selected Deal against its JetLearn Deal Source
    if source_col and src_for_sel != "Unknown":
    ...
    # ---- KPIs
    c1, c2, c3 = st.columns(3)
    with c1: st.metric("JetLearn Deal Source", src_for_sel)
    with c2: st.metric("Deal Count (Created in window)", deal_count)
    with c3: st.metric("Enrollment Count", enrol_count)

    st.metric("Share of Selected Deal within Source", f"{pct_within_src:.2f}%")
    ...
    row = {
        "Deal Name": sel_deal,
        "JetLearn Deal Source": src_for_sel,
    ...
    if record_id_col:
        # include records for reference if multiple rows exist for same deal
        rec_ids = df.loc[sel_mask & (m_create | enrol_mask), record_id_col].astype(str).unique().tolist()
        row["Record IDs (in window)"] = ", ".join(rec_ids[:50])  # cap for display
...
try:
    ...
    import streamlit as _st  # avoid shadowing
