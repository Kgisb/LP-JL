...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report", "Pipeline", "Sales Activity", "Deal stage", "Original source", "Referral / No-Referral", "Lead mix", "Referral performance", "Slow Working Deals", "Activity concentration", "Deal type" ],
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail"],
    }
...
# --- Performance / Deal type (explicit master gate) ---
try:
    if 'master' in globals() and master == "Performance" and 'view' in globals() and view == "Deal type":
        _render_performance_deal_type(
            df_f=df_f,
            create_col=create_col if 'create_col' in globals() else None,
            pay_col=pay_col if 'pay_col' in globals() else None,
            source_col=source_col if 'source_col' in globals() else None,
            deal_name_col=find_col(df, ["Deal Name","Deal name","Name","Deal","Title"]) if 'df' in globals() else None,
            parent_phone_col=find_col(df, ["Parent Phone Number","Parent Phone","Phone","Parent Contact Number","Primary Phone"]) if 'df' in globals() else None,
            parent_email_col=find_col(df, ["Parent Email ID","Parent Email","Email","Parent Email Address","Primary Email"]) if 'df' in globals() else None,
            trial_scheduled_col=find_col(df, ["Trial Scheduled Date","First Trial Scheduled Date","First Calibration Scheduled Date","Calibration Scheduled Date"]) if 'df' in globals() else None,
            trial_rescheduled_col=find_col(df, ["Trial Rescheduled Date","Calibration Rescheduled Date"]) if 'df' in globals() else None,
            calibration_booked_col=find_col(df, ["Calibration Booking Date","Calibration Booked Date","Calibration Booked"]) if 'df' in globals() else None,
            calibration_done_col=find_col(df, ["Calibration Done Date","Trial Done Date","Trial Completed Date","Calibration Completed Date"]) if 'df' in globals() else None,
        )
except Exception as e:
    import streamlit as st
    st.error(f"Deal type rendering error: {e}")
...
# === Deal type (with Funnel) renderer inserted ===

# ======================
# Deal type — Performance pill (drop-in, v2 with Funnel)
# ======================
#
# Integration steps (copy-paste into your existing app file):
# 1) Paste this whole block near your other renderers (e.g., after _render_performance_sales_activity).
# 2) In your sidebar MASTER_SECTIONS mapping, append "Deal type" under "Performance":
#       "Performance": [..., "Deal type"]
# 3) In the main view switcher where you route views, add:
#       elif view == "Deal type":
#           _render_performance_deal_type(
#               df_f=df_f,
#               create_col=create_col,
#               pay_col=pay_col,
#               source_col=source_col,
#               deal_name_col=find_col(df, ["Deal Name","Deal name","Name","Deal","Title"]),
#               parent_phone_col=find_col(df, ["Parent Phone Number","Parent Phone","Phone","Parent Contact Number","Primary Phone"]),
#               parent_email_col=find_col(df, ["Parent Email ID","Parent Email","Email","Parent Email Address","Primary Email"]),
#               # Optional: explicitly pass your milestone columns if you want
#               trial_scheduled_col=find_col(df, ["Trial Scheduled Date","First Trial Scheduled Date","First Calibration Scheduled Date","Calibration Scheduled Date"]),
#               trial_rescheduled_col=find_col(df, ["Trial Rescheduled Date","Calibration Rescheduled Date"]),
#               calibration_booked_col=find_col(df, ["Calibration Booking Date","Calibration Booked Date","Calibration Booked"]),
#               calibration_done_col=find_col(df, ["Calibration Done Date","Trial Done Date","Trial Completed Date","Calibration Completed Date"]),
#           )
#
# Notes:
# - Uses your existing helpers: find_col, coerce_datetime (if present). Otherwise, falls back to safe versions.
# - Does NOT modify existing logic. It's a new pill only.
# - MTD vs Cohort logic:
#     • MTD: Create Date must be in the current month; milestone date must fall in the selected range.
#     • Cohort: milestone date must fall in the selected range (Create Date not required in-month).
# - Output table grouped by JetLearn Deal Source adds funnel columns:
#     • Trial Scheduled
#     • Trial Rescheduled
#     • Calibration Booked
#     • Calibration Done
#   Counting is by unique Deal Name hitting that milestone within the chosen date window.
#
import pandas as _pd
import numpy as _np
from datetime import date as _date

def _render_performance_deal_type(
    df_f: _pd.DataFrame,
    create_col: str | None,
    pay_col: str | None,
    source_col: str | None,
    deal_name_col: str | None = None,
    parent_phone_col: str | None = None,
    parent_email_col: str | None = None,
    trial_scheduled_col: str | None = None,
    trial_rescheduled_col: str | None = None,
    calibration_booked_col: str | None = None,
    calibration_done_col: str | None = None,
):
    import streamlit as st

    st.subheader("Performance — Deal type (with Funnel)")

    if df_f is None or getattr(df_f, "empty", True):
        st.info("No data in scope."); return

    # --- Resolve columns robustly ---
    deal_col   = deal_name_col    or _deal_type_find_col(df_f, ["Deal Name","Deal name","Name","Deal","Title"])
    src_col    = source_col       or _deal_type_find_col(df_f, ["JetLearn Deal Source","Deal Source","Source","Original source","Original traffic source"])
    phone_col  = parent_phone_col or _deal_type_find_col(df_f, ["Parent Phone Number","Parent Phone","Phone","Parent Contact Number","Primary Phone"])
    email_col  = parent_email_col or _deal_type_find_col(df_f, ["Parent Email ID","Parent Email","Email","Parent Email Address","Primary Email"])

    trial_s_col = trial_scheduled_col   or _deal_type_find_col(df_f, ["Trial Scheduled Date","First Trial Scheduled Date","First Calibration Scheduled Date","Calibration Scheduled Date"])
    trial_r_col = trial_rescheduled_col or _deal_type_find_col(df_f, ["Trial Rescheduled Date","Calibration Rescheduled Date"])
    calib_b_col = calibration_booked_col or _deal_type_find_col(df_f, ["Calibration Booking Date","Calibration Booked Date","Calibration Booked"])
    calib_d_col = calibration_done_col   or _deal_type_find_col(df_f, ["Calibration Done Date","Trial Done Date","Trial Completed Date","Calibration Completed Date"])

    if deal_col is None:
        st.error("Could not find a Deal Name column."); return
    if src_col is None:
        st.error("Could not find JetLearn Deal Source column."); return

    # Optional date columns
    _create_dt = _dt_safe_to_datetime(df_f[create_col]) if (create_col and create_col in df_f.columns) else _pd.Series(_pd.NaT, index=df_f.index)
    _pay_dt    = _dt_safe_to_datetime(df_f[pay_col])    if (pay_col and pay_col in df_f.columns)     else _pd.Series(_pd.NaT, index=df_f.index)

    # Milestone dates
    _trial_s_dt = _dt_safe_to_datetime(df_f[trial_s_col]) if (trial_s_col and trial_s_col in df_f.columns) else _pd.Series(_pd.NaT, index=df_f.index)
    _trial_r_dt = _dt_safe_to_datetime(df_f[trial_r_col]) if (trial_r_col and trial_r_col in df_f.columns) else _pd.Series(_pd.NaT, index=df_f.index)
    _calib_b_dt = _dt_safe_to_datetime(df_f[calib_b_col]) if (calib_b_col and calib_b_col in df_f.columns) else _pd.Series(_pd.NaT, index=df_f.index)
    _calib_d_dt = _dt_safe_to_datetime(df_f[calib_d_col]) if (calib_d_col and calib_d_col in df_f.columns) else _pd.Series(_pd.NaT, index=df_f.index)

    # --- Controls ---
    mode = st.radio("Counting mode", ["MTD","Cohort"], index=0, horizontal=True, key="dealtype_mode")
    today = _date.today()
    ...
    with st.container():
        c1,c2,c3 = st.columns([1,1,2])
        with c1:
            st.caption(f"MTD month: **{m_start} → {m_end}**")
        with c2:
            dr = st.date_input("Date range", value=(m_start, m_end), key="dealtype_range")
        with c3:
            names = sorted(df_f[deal_col].dropna().astype(str).unique().tolist())
            default = names[:20] if len(names) > 20 else names
            picked = st.multiselect("Select Deal Name(s)", options=names, default=default, key="dealtype_names")
    ...
    end_dt   = _pd.to_datetime(end) + _pd.Timedelta(days=1) - _pd.Timedelta(seconds=1)

    in_create_month = _create_dt.dt.date.between(m_start, m_end) if _create_dt.notna().any() else _pd.Series(True, index=df_f.index)

    # Milestone windows (each milestone uses its own date column)
    in_range_trial_s = _trial_s_dt.between(start_dt, end_dt) if _trial_s_dt.notna().any() else _pd.Series(False, index=df_f.index)
    in_range_trial_r = _trial_r_dt.between(start_dt, end_dt) if _trial_r_dt.notna().any() else _pd.Series(False, index=df_f.index)
    in_range_calib_b = _calib_b_dt.between(start_dt, end_dt) if _calib_b_dt.notna().any() else _pd.Series(False, index=df_f.index)
    in_range_calib_d = _calib_d_dt.between(start_dt, end_dt) if _calib_d_dt.notna().any() else _pd.Series(False, index=df_f.index)

    # Base selection by Deal Name
    base = df_f[deal_col].astype(str).isin(picked)

    def _mode_mask(milestone_mask: _pd.Series):
        if mode == "MTD":
            return base & in_create_month & milestone_mask
        else:
            return base & milestone_mask

    mask_any = base & (in_create_month if mode=="MTD" else True)

    d = df_f.loc[mask_any].copy()
    if d.empty:
        st.info("No rows match the selection/window."); return

    # --- Normalize strings ---
    d["_deal"] = d[deal_col].astype(str)
    d["_src"]  = (d[src_col].astype(str) if src_col in d.columns else "Unknown").fillna("Unknown").str.strip()

    phone_series = d[phone_col].astype(str).str.strip() if (phone_col and phone_col in d.columns) else _pd.Series("", index=d.index)
    email_series = d[email_col].astype(str).str.strip() if (email_col and email_col in d.columns) else _pd.Series("", index=d.index)

    # Has Phone/Email by deal (deduplicated per deal)
    d_base_unique = d.drop_duplicates("_deal").copy()
    d_base_unique["_has_phone"] = d_base_unique[phone_col].astype(str).str.strip().ne("") if (phone_col and phone_col in d_base_unique.columns) else False
    d_base_unique["_has_email"] = d_base_unique[email_col].astype(str).str.strip().ne("") if (email_col and email_col in d_base_unique.columns) else False

    total_selected_deals = int(d_base_unique["_deal"].nunique())

    # Helper to compute unique-deal hits per source for a given milestone mask
    def _milestone_counts(milestone_mask: _pd.Series):
        dd = df_f.loc[_mode_mask(milestone_mask) & df_f[deal_col].astype(str).isin(d_base_unique["_deal"]) , [deal_col, src_col]].copy()
        if dd.empty:
            return _pd.Series(dtype=int)
        dd["_deal"] = dd[deal_col].astype(str)
        dd["_src"]  = dd[src_col].fillna("Unknown").astype(str).str.strip()
        dd = dd.drop_duplicates(["_src","_deal"])
        return dd.groupby("_src")["_deal"].nunique()

    trial_s_counts = _milestone_counts(in_range_trial_s).rename("Trial Scheduled")
    trial_r_counts = _milestone_counts(in_range_trial_r).rename("Trial Rescheduled")
    calib_b_counts = _milestone_counts(in_range_calib_b).rename("Calibration Booked")
    calib_d_counts = _milestone_counts(in_range_calib_d).rename("Calibration Done")

    # Deals per source (any rows in mask_any)
    grp = d.groupby("_src")
    deals_per_src = grp["_deal"].nunique().rename("Deals")

    # Phone/Email counts per source (count unique deals with non-empty fields)
    dedup_src_deal = d.drop_duplicates(["_src","_deal"]).copy()
    has_phone_per_src = dedup_src_deal.groupby("_src").apply(lambda x: x["_deal"][ (x["_deal"].isin(d_base_unique.loc[d_base_unique["_has_phone"], "_deal"])) ].nunique()).rename("Has Phone")
    has_email_per_src = dedup_src_deal.groupby("_src").apply(lambda x: x["_deal"][ (x["_deal"].isin(d_base_unique.loc[d_base_unique["_has_email"], "_deal"])) ].nunique()).rename("Has Email")

    # Combine
    out = _pd.concat([deals_per_src, trial_s_counts, trial_r_counts, calib_b_counts, calib_d_counts, has_phone_per_src, has_email_per_src], axis=1).fillna(0).astype({"Deals": int, "Trial Scheduled": int, "Trial Rescheduled": int, "Calibration Booked": int, "Calibration Done": int, "Has Phone": int, "Has Email": int}).reset_index().rename(columns={"_src":"JetLearn Deal Source"})

    # Percent of selected
    if total_selected_deals > 0:
        out["% of Selected"] = (out["Deals"] / float(total_selected_deals) * 100.0).round(1)
    else:
        out["% of Selected"] = 0.0

    # Order
    out = out.sort_values(["Deals","Calibration Done","Calibration Booked","Trial Scheduled"], ascending=[False,False,False,False], kind="stable").reset_index(drop=True)

    # --- KPIs ---
    c1,c2,c3,c4,c5 = st.columns(5)
    with c1: st.metric("Selected deal names", total_selected_deals)
    with c2: st.metric("Sources hit", int(out.shape[0]))
    with c3: st.metric("Trial Scheduled", int(out["Trial Scheduled"].sum()))
    with c4: st.metric("Cal. Booked", int(out["Calibration Booked"].sum()))
    with c5: st.metric("Cal. Done", int(out["Calibration Done"].sum()))

    # --- Table + download ---
    st.dataframe(out, use_container_width=True, hide_index=True)
    st.download_button(
        "Download CSV — Deal type (funnel by source)",
        out.to_csv(index=False).encode("utf-8"),
        file_name="deal_type_funnel_by_source.csv",
        mime="text/csv",
        key="dl_dealtype_funnel_by_source"
    )
