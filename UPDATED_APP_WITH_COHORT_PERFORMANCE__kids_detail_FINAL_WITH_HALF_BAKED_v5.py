...
# === Half Baked renderer (EOF) ===
def _render_marketing_half_baked(df):
    import pandas as _pd, numpy as _np, streamlit as st, altair as alt
    ...
    def _to_dt(s): return _pd.to_datetime(s, errors="coerce", dayfirst=True)
    ...
    for c in [enroll_col, cal_book_col, cal_slot_col, cal_resch_col, cal_done_col]:
        if c and c in df_sel.columns: df_sel[c] = _to_dt(df_sel[c])
    if enroll_col and enroll_col in df_all.columns: df_all[enroll_col] = _to_dt(df_all[enroll_col])
    ...
    def _nonempty(s): return s.astype(str).str.strip().ne("").fillna(False)
    ...
    total_sel = int(sel_counts.sum()); total_all = int(all_counts.sum())
